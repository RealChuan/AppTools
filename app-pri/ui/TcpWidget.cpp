...
TcpWidget::~TcpWidget()
{
    if(!d->serverClient.isEmpty()){
        qDeleteAll(d->serverClient);
        d->serverClient.clear();
    }
    saveConfig();
    delete ui;
    delete d;
...
void TcpWidget::openTcp()
{
    ...
    else if(ui->listen->text()=="断开")
    {
        ...
        d->sendTime->stop();
        //qDebug() << "openTcp: " << d->serverClient.size();
        for(int i=0; i<d->serverClient.size(); i++)//断开所有连接
        {
...
{
    qDebug() << "disconnected: " << d->serverClient.size();
    QTcpSocket *currentClient = qobject_cast<QTcpSocket*>(sender());
    if(currentClient == nullptr) {
        qDebug() << "disconnected return" ;
        return;
    }
