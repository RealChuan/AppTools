                     ...
                     const double margin)
{
    ...
    calculateArc(center, radius0, ply[0], ply[1], isInTop, path_temp, startAngle, endAngle, deltaAngle);
...
{
    setArc(arc);
    calculateAllArc(cache(), d->arcPath, d->shape, margin());
}
...
inline QPolygonF calculateCache(const Arc &arch)
{
    ...
    QPointF pt11 = findAnotherPtOfLine(arch.center, arch.maxRadius,
                                       (arch.startAngle + arch.endAngle) / 2.0);
...
inline bool checkArc(const Arc &arch, const double margin)
{
    ...
    return arch.minRadius >= minLen
            && arch.maxRadius - arch.minRadius >= minLen;
}
...
QRectF GraphicsArcItem::boundingRect() const
{
    ...
    QPolygonF pts = d->arcPath.toFillPolygon() + cache();
...
void GraphicsArcItem::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    setClickedPos(event->scenePos());
    QPolygonF pts_tmp = cache();
    double distance = Graphics::distance(d->arch.center, point);
    ...
    case BasicGraphicsItem::None: {
        ...
        case InEdge0:
            ...
            for (int i = 0; i < 3; ++i)
                pts_tmp[i] = lineSetLength(d->arch.center, pts_tmp[i], distance);
            break;
        case InEdge1:
            setMyCursor(d->arch.center, event->scenePos());
            pts_tmp[3] = lineSetLength(d->arch.center, pts_tmp[3], distance);
            break;
        case InEdgeH:
        case InEdgeL:{
            Arc arch = d->arch;
...
void GraphicsArcItem::hoverMoveEvent(QGraphicsSceneHoverEvent *event)
{
    ...
    if(qAbs(Graphics::distance(point, d->arch.center) - d->arch.minRadius) < margin() / 3) {
        ...
        setMyCursor(d->arch.center, point);
    }else if(qAbs(Graphics::distance(point, d->arch.center) - d->arch.maxRadius) < margin() / 3) {
    ...
    }else if(d->arcPath.contains(point)){
...
void GraphicsArcItem::pointsChanged(const QPolygonF& ply)
{
    ...
    case 2:
        ...
        break;
    case 3:{
        if(!calculateHalfArc(ply, d->cachePath))
            return;
        QPolygonF polygon = d->cachePath.toFillPolygon() + ply;
        if(!rect.contains(polygon.boundingRect()))
            return;
        setCache(ply);
    } break;
    case 4:{
        if(!calculateAllArc(ply, d->arcPath, d->shape, margin()))
            return;
        QPolygonF polygon = d->shape.toFillPolygon() + ply;
        if(!rect.contains(polygon.boundingRect()))
            return;
...
void GraphicsArcItem::showHoverArc(const QPolygonF &ply)
{
    ...
    case 3:
        ...
        if(Graphics::distance(ply[1], ply[2]) < margin())
            return;
        calculateHalfArc(ply, d->cachePath);
        break;
    case 4: calculateAllArc(ply, d->cachePath, d->shape, margin()); break;
    default: return;
    }
